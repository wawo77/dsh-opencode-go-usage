...
class GoUsageService {
  ...
  /**
   * Whether a session's provider is accounted by the local ledger.
   *
   * **不限于 Go** —— token 统计和用哪家套餐无关。原来只记 Go，于是给
   * DeepSeek 官方加上适配器之后，"今日 token"那行会是空的：界面显示了余额，
   * 却说自己今天一个 token 都没用，明显自相矛盾。
   *
   * 判据：是 Go 路由，**或者有适配器认领它**（= 我们声明支持这家）。
   * 没有适配器的 provider 仍不记 —— 那种情况下界面本来就走"只显示本地统计"
   * 的中性提示，账本里留白反而一致。
   */
  isAccountedProvider(provider) {
    if (typeof provider !== 'string' || provider === '') return false
    if (/^opencode/i.test(provider)) return true
    if (this.goRoutes().some((route) => route.id === provider)) return true
    return adapterFor({ providerId: provider }) !== undefined
  }
  ...
  record(provider, model, usage, atMs) {
    if (!this.isAccountedProvider(provider)) return
    const bucket = {
